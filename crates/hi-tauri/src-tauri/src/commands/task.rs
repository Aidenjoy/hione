...
pub async fn list_tasks(state: State<'_, AppState>) -> Result<Vec<TaskRecord>, String> {
    ...
    match work_dir {
        Some(wd) => session::get_task_records(&wd)
            .await
...
pub async fn push_task(
    ...
    let client =
        ipc_client::IpcClient::with_hione_dir(session_info.socket_path, session_info.hione_dir);
...
#[tauri::command]
pub async fn check_agent(state: State<'_, AppState>, name: String) -> Result<bool, String> {
    let work_dir = {
        let current = state.current_work_dir.lock().unwrap();
        current.clone()
    };
    ...
    client.check_agent(&name).await.map_err(|e| e.to_string())
}
